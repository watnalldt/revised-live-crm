...
def contracts_by_type(user, contract_type):
    ...
    return Contract.objects.filter(client__account_manager=user, contract_type=contract_type).count()
...
def directors_approval(user, is_directors_approval):
    ...
    return Contract.objects.filter(client__account_manager=user, is_directors_approval=is_directors_approval).count()
...
def contracts_by_utility(user, utility_type):
    ...
    return Contract.objects.filter(client__account_manager=user, utility__utility=utility_type).count()

@register.simple_tag
def contracts_by_supplier(user, supplier_name):
    """Returns total number of contracts with a specific supplier for a given user."""
    return Contract.objects.filter(client__account_manager=user, supplier__supplier=supplier_name).count()

@register.simple_tag
def contracts_by_status(user, contract_status):
    """Returns total number of contracts with a specific supplier for a given user."""
    return Contract.objects.filter(client__account_manager=user, contract_status=contract_status).count()
...
def expired_contracts(user):
    ...
    return Contract.objects.filter(client__account_manager=user, contract_end_date__lt=end_date).count()
